  ...
  return (
    <nav className="glass sticky top-0 z-50 border-b border-white/5">
      <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
        <a href="/" className="text-xl font-bold text-gradient hover:opacity-80 transition">
          AI Blog
        </a>

        {/* Desktop */}
        <div className="hidden md:flex items-center space-x-6">
          <a href="/articles" className="text-gray-300 hover:text-white transition text-sm font-medium">
            Articles
          </a>
          {admin && (
            <a href="/admin" className="text-yellow-400 hover:text-yellow-300 transition text-sm font-medium flex items-center gap-1">
          ...
          {user ? (
            ...
            <a href="/login" className="btn-primary text-white text-sm px-4 py-2 rounded-lg">
              Đăng nhập
            </a>
          )}
        </div>
      ...
      {menuOpen && (
        <div className="md:hidden glass border-t border-white/5 px-4 py-4 space-y-3">
          <a href="/articles" className="block text-gray-300 hover:text-white">Articles</a>
          {admin && <a href="/admin" className="block text-yellow-400">Admin</a>}
          {user ? (
            <>
            ...
            <a href="/login" className="block text-indigo-400">Đăng nhập</a>
          )}
        </div>
      )}
    </nav>
